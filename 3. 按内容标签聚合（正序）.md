...
.action{$tagBlocksList:= (queryBlocks "SELECT * FROM blocks WHERE content LIKE '%?%' AND subtype LiKE '%h%' ORDER BY created" $searchTag)}
...
{{SELECT * FROM blocks WHERE parent_id='.action{$tagBlock.ID} ORDER BY created'}}
	.action{end}
.action{end}
